...
├── final data/               not in this repo: 4 raw JSONL dumps, ~2.2 GB (see Data below)
└── data/                     not in this repo: everything computed from the raw dumps
    ├── parquet/                   ingested + cleaned corpus, as Parquet
    ├── derived/                   embeddings, LLM labels, distilled predictions, gold labels, KG edges/nodes
    └── cache/                     cached LLM API responses, so a rerun does not repeat billed calls
```
...
The Groq-dependent stages (`04_llm_annotate.R`, `13_graph.R`, and the optional `04b_selfconsistency.R` / `06_variants.R`) are rate-limited by the free tier's daily quota and can't be finished in one sitting. Two stages in the middle of the chain, human coding of the sampled comments and the manual groundedness check of the knowledge graph, are conducted by hand rather than by script; every other stage is fully scripted and reproducible from the outputs this repository already ships in `output/`.

## 🧭 Data

Two folders are not tracked in this repository, both too large for git:

- **`final data/`** (~2.2 GB): the raw input, four JSONL dumps (one per subreddit, six months), sourced from the Arctic Shift bulk archive rather than the Reddit API.
- **`data/`** (~1.7 GB): everything the pipeline computes from that raw input on the way to `output/`, split into `parquet/` (the ingested and cleaned corpus), `derived/` (embeddings, LLM labels, distilled predictions, gold labels, knowledge-graph edges and nodes), and `cache/` (LLM API responses, cached so a rerun does not repeat calls already billed against quota).

Comments are pseudonymous rather than anonymous, and a verbatim quotation is searchable back to its author, so the project's own ethics practice is to redistribute by comment identifier rather than by text, not to publish the raw dumps. `output/tables/` and `output/listings/` ship every number and intermediate result the analysis produced, so the full analysis is auditable even without either folder.
